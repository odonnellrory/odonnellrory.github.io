...
Verified the workflow sequence: checkout → setup Python → install deps → mkdocs build --strict → upload artifact → deploy to Pages.

Confirmed that after fixing, the site built correctly with plugins enabled.

Sources

MkDocs Material — Publishing your site (GitHub Pages with Actions)

GitHub Docs — Using GitHub Actions to deploy Pages

actions/deploy-pages (official action)

mkdocs-awesome-pages-plugin (PyPI)
