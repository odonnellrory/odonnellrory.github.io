...
* Fixed by nesting headings properly:
  ...
  ```
* Also extended `mkdocs.yml` to include deeper levels:
...
* Why pipx wasn’t the right tool for MkDocs plugins, and why virtualenvs are the clean solution.
* How the heading hierarchy drives MkDocs’ TOC, and how to configure `toc_depth` for multi-level outlines.
